...
class Student:
    def __init__(self, root):
        ...
        year_combo=ttk.Combobox(current_course_frame,textvariable=self.var_year,font=("times new roman",13,"bold"),state="read only",width=18)
        ...
        mentor_entry=ttk.Entry(class_Student_frame,textvariable=self.var_mentor,width=20,font=("times new roman",13,"bold"))
        ...
        update_btn=Button(btn_frame,text="Update",command=self.update_data,width=17,font=("times new roman",13,"bold"),bg="gold",fg="white")
        ...
        take_photo_btn=Button(btn_frame1,command=self.generate_dataset,text="Take Photo Sample",width=35,font=("times new roman",13,"bold"),bg="lightgreen",fg="white")
        ...
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id.get()=="":
            ...
            try:
                ...
                if Update>0:
                    conn=mysql.connector.connect(host="localhost",username="root",password="MySQLAman",database="student_attendence")
                    my_cursor=conn.cursor()
                    my_cursor.execute("update student set Dep=%s,Course=%s,Year=%s,Semester=%s,Name=%s,Division=%s,Roll=%s,Gender=%s,Dob=%s,Email=%s,Phone=%s,Address=%s,Mentor=%s,PhotoSample=%s where Student_id=%s",(
    ...
    def generate_dataset(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id.get()=="":
            ...
            try:
                ...
                face_classifier=cv2.CascadeClassifier("D:/SAM/src/haarcascade_frontalface_default.xml")
